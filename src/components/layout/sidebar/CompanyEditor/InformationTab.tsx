  ...
  return <Form {...form}>
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <h3 className="text-sm font-medium">Vagas</h3>
          
          <div className="flex flex-wrap gap-1">
            {selectedJobPositions.map(position => <Badge key={position} variant="secondary" className="flex items-center gap-1 bg-primary/10">
                <BriefcaseBusiness className="h-3 w-3" />
                {position}
                <X className="h-3 w-3 ml-1 cursor-pointer" onClick={() => removeJobPosition(position)} />
              </Badge>)}
          </div>
              ...
              <Select onValueChange={handleJobPositionChange}>
                ...
                <SelectContent>
                  <ScrollArea className="h-[200px]">
                    ...
                    {availableJobPositions.map(job => <SelectItem key={job} value={job}>{job}</SelectItem>)}
                  </ScrollArea>
                </SelectContent>
              </Select>
            </div>
        ...
        field
              ...
              <div className="grid grid-cols-3 gap-2">
                <Button type="button" variant={selectedUrgency === 'Baixa' ? 'default' : 'outline'} className="w-full flex items-center justify-start gap-2 h-12" onClick={() => form.setValue('urgency', 'Baixa')}>
                  <div className="w-3 h-3 rounded-full bg-green-500"></div>
                  <span>Baixa</span>
                </Button>
                <Button type="button" variant={selectedUrgency === 'Média' ? 'default' : 'outline'} className="w-full flex items-center justify-start gap-2 h-12" onClick={() => form.setValue('urgency', 'Média')}>
                  <div className="w-3 h-3 rounded-full bg-yellow-500"></div>
                  <span>Média</span>
                </Button>
                <Button type="button" variant={selectedUrgency === 'Alta' ? 'default' : 'outline'} className="w-full flex items-center justify-start gap-2 h-12" onClick={() => form.setValue('urgency', 'Alta')}>
        ...
        <div className="space-y-2">
          <h3 className="text-sm font-medium">Decorrer</h3>
          
          <FormField control={form.control} name="inProgress" render={({
          field
        }) => <Select value={field.value || ''} onValueChange={value => form.setValue('inProgress', value === 'none' ? '' : value)}>
                <SelectTrigger>
        ...
        <div className="space-y-2">
          ...
          <FormField control={form.control} name="selector" render={({
          field
        }) => <Select value={field.value || ''} onValueChange={value => form.setValue('selector', value === 'none' ? '' : value)}>
                <SelectTrigger>
